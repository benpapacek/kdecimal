...
//    }
     ...
     * @see .bitLength
     */
//    constructor(numBits: Int, rnd: Random) {
...
//    }
     ...
     * performance when `val == this`.
     *
     * @implNote Compared to [.multiply], an implementation's
     ...
     * @see .multiply
     */
    fun parallelMultiply(`val`: BigInteger): BigInteger {
        return multiply(`val`, false, true, 0)
    }

    /**
     * Returns a BigInteger whose value is `(this * val)`.  If
     ...
     * @return `this * val`
     */
    private fun multiply(`val`: BigInteger, isRecursion: Boolean, parallel: Boolean, depth: Int): BigInteger {
...
//                    if (bitLength(mag, mag.size).toLong() + bitLength(
//                            `val`.mag,
//                            `val`.mag.size
//                        ).toLong() > 32L * MAX_MAG_LENGTH
//                    ) {
//                        reportOverflow()
//                    }
//                }
...
//    private abstract class RecursiveOp private constructor(internal val parallel: Boolean, depth: Int) :
...
//        }
//
////        protected fun forkOrInvoke(): RecursiveTask<BigInteger> {
////            if (parallel && depth <= parallelForkDepthThreshold)
////                fork()
////            else
////                invoke()
////            return this
////        }
//
//        private class RecursiveMultiply(
//            private val a: BigInteger,
//            private val b: BigInteger,
//            parallel: Boolean,
//            depth: Int
//        ) : RecursiveOp(parallel, depth) {
//
//            public override fun compute(): BigInteger {
//                return a.multiply(b, true, parallel, depth.toInt())
//            }
//        }
//
//        private class RecursiveSquare(private val a: BigInteger, parallel: Boolean, depth: Int) :
//            RecursiveOp(parallel, depth) {
//
//            public override fun compute(): BigInteger {
//                return a.square(true, parallel, depth.toInt())
//            }
//        }
//
//        companion object {
...
//             * multiply and square.
//             */
//            private val PARALLEL_FORK_DEPTH_THRESHOLD = calculateMaximumDepth(ForkJoinPool.getCommonPoolParallelism())
//
//            private fun calculateMaximumDepth(parallelism: Int): Int {
//                return 32 - Integer.numberOfLeadingZeros(parallelism)
//            }
//
////            private val parallelForkDepthThreshold: Int
////                get() = if (Thread.currentThread() is ForkJoinWorkerThread) {
////                    calculateMaximumDepth(fjwt.getPool().getParallelism())
////                } else {
////                    PARALLEL_FORK_DEPTH_THRESHOLD
////                }
//
////             fun multiply(
////                a: BigInteger,
...
////                depth: Int
////            ): RecursiveTask<BigInteger> {
////                return RecursiveMultiply(a, b, parallel, depth).forkOrInvoke()
////            }
////
////             fun square(a: BigInteger, parallel: Boolean, depth: Int): RecursiveTask<BigInteger> {
////                return RecursiveSquare(a, parallel, depth).forkOrInvoke()
////            }
//        }
//    }
     ...
     */
    private fun square(isRecursion: Boolean = false, parallel: Boolean = false, depth: Int = 0): BigInteger {
        if (signum == 0) {
            return ZERO
        }
        val len = mag.size
...
//                //
//                if (!isRecursion) {
//                    if (bitLength(mag, mag.size) > 16L * MAX_MAG_LENGTH) {
//                        reportOverflow()
//                    }
//                }
//
...
//            }
     ...
     */
//    private fun squareToomCook3(parallel: Boolean, depth: Int): BigInteger {
//        var depth = depth
//        val len = mag.size
//
...
//
//        depth++
//        val v0_fork = RecursiveOp.square(a0, parallel, depth)
//        da1 = a2.add(a0)
//        val vm1_fork = RecursiveOp.square(da1.subtract(a1), parallel, depth)
//        da1 = da1.add(a1)
//        val v1_fork = RecursiveOp.square(da1, parallel, depth)
...
//    }
    ...
    /** Long division  */
    fun divideAndRemainderKnuth(`val`: BigInteger): Array<BigInteger> {
//        val result = arrayOfNulls<BigInteger>(2)
        ...
        val x = q.toBigInteger(if (this.signum == `val`.signum) 1 else -1)
        val y = r!!.toBigInteger(this.signum)
        return arrayOf(x, y)
    }
        ...
        if (powersOfTwo > 0) {
            ...
            if (remainingBits == 1) {  // Nothing left but +/- 1?
                return if (signum < 0 && exponent and 1 == 1) {
                    NEGATIVE_ONE!!.shiftLeft(bitsToShift)
                } else {
                    ONE!!.shiftLeft(bitsToShift)
                }
            }
        } else {
            remainingBits = partToSquare.bitLength()
        ...
        if (partToSquare.mag.size == 1 && scaleFactor <= 62) {
            ...
            return if (powersOfTwo > 0) {
                if (bitsToShift + scaleFactor <= 62) { // Fits in long?
                    ...
                    valueOf(result * newSign)!!.shiftLeft(bitsToShift)!!
                }
            } else {
     ...
     * @param  m the modulus.
     * @return `this<sup>exponent</sup> mod m`
     ...
     */
        ...
        val inv = -MutableBigInteger.inverseMod64(n0.toLong())
    ...
    private fun shiftRightImpl(n: Int): BigInteger {
        ...
        if (nBits == 0) {
            ...
            Objects.checkFromToIndex(i, numIter + i, newMag.size)
...
//    }
     ...
     * The value must be non-negative. If `digits <= 0` no padding
    ...
    /**
     * Save the `BigInteger` instance to a stream.  The magnitude of a
     ...
     * `bitCount`, `bitLength`, `lowestSetBit`, and
     * `firstNonzeroByteNum`, respectively.  These values are compatible
...
//    }
    ...
    companion object {
         ...
         */
        val BURNIKEL_ZIEGLER_THRESHOLD = 80
         ...
         */
        val BURNIKEL_ZIEGLER_OFFSET = 40
...
//        @Volatile
        ...
        private fun subtract(`val`: Long, little: IntArray): IntArray {
            ...
            if (highWord == 0) {
                ...
                if (little.size == 1) {
                    val difference = (`val` and LONG_MASK) - (little[0].toLong() and LONG_MASK)
                    ...
                    var difference = (`val` and LONG_MASK) - (little[1].toLong() and LONG_MASK)
                    result[1] = difference.toInt()
         ...
         *
         */
//        private fun multiplyToomCook3(a: BigInteger, b: BigInteger, parallel: Boolean, depth: Int): BigInteger {
//            var depth = depth
//            val alen = a.mag.size
//            val blen = b.mag.size
//
...
//
//            depth++
//            val v0_task = RecursiveOp.multiply(a0, b0, parallel, depth)
//            da1 = a2.add(a0)
//            db1 = b2.add(b0)
//            val vm1_task = RecursiveOp.multiply(da1.subtract(a1), db1.subtract(b1), parallel, depth)
//            da1 = da1.add(a1)
//            db1 = db1.add(b1)
//            val v1_task = RecursiveOp.multiply(da1, db1, parallel, depth)
...
//        }
        ...
        private fun implSquareToLenChecks(x: IntArray, len: Int, z: IntArray, zlen: Int) {
            require(len >= 1) { "invalid input length: $len" }
            require(len <= x.size) {
                "input length out of bound: " +
                        len + " > " + x.size
            }
            require(len * 2 <= z.size) {
                "input length out of bound: " +
                        len * 2 + " > " + z.size
            }
            require(zlen >= 1) { "invalid input length: $zlen" }
            require(zlen <= z.size) {
                "input length out of bound: " +
                        len + " > " + z.size
            }
        ...
        internal fun primitiveRightShift(a: IntArray, len: Int, n: Int) {
            Objects.checkFromToIndex(0, len, a.size)
        ...
        private fun implMontgomeryMultiplyChecks(a: IntArray, b: IntArray, n: IntArray, len: Int, product: IntArray?) {
            require(len % 2 == 0) { "input array length must be even: $len" }
            ...
            ) { "input array length out of bound: $len" }
        }
     ...
     */
        ...
        private fun implMulAddCheck(out: IntArray, `in`: IntArray, offset: Int, len: Int, k: Int) {
            require(len <= `in`.size) { "input length is out of bound: " + len + " > " + `in`.size }
            require(offset >= 0) { "input offset is invalid: $offset" }
            require(offset <= out.size - 1) { "input offset is out of bound: " + offset + " > " + (out.size - 1) }
            require(len <= out.size - offset) { "input len is out of bound: " + len + " > " + (out.size - offset) }
        }
        ...
        //    @ForceInline
        //    @IntrinsicCandidate
        private fun shiftRightImplWorker(
            newArr: IntArray,
            oldArr: IntArray,
            newIdx: Int,
            shiftCount: Int,
        ...
        /**
         * If `numZeros > 0`, appends that many zeros to the
         ...
         */
        private fun padWithZeros(buf: StringBuilder, numZeros: Int) {
        ...
        private val NUM_ZEROS = 63

        /* ZEROS is a string of NUM_ZEROS consecutive zeros. */
        private val ZEROS = "0".repeat(NUM_ZEROS)
     ...
     */
        ...
        private val longRadix = arrayOf<BigInteger?>(
            null,
            null,
 ...
 * provided by [ Character.digit][Character.digit].  The String may not contain any extraneous
